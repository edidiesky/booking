...
const Registerurl = `${process.env.VITE_API_BASE_URLS}/api/v1/auth/register`;
const Loginurl = `${process.env.VITE_API_BASE_URLS}/api/v1/auth/login`;
...
export const getAllCustomer = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.get(`${process.env.VITE_API_BASE_URLS}/api/v1/user?page=${userpage}`, config);
      return data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
        error.response && error.response.data.message
...
export const getUserStats = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.get(`${process.env.VITE_API_BASE_URLS}/api/v1/user/stats`, config);
  ...
  async (name, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${process.env.VITE_API_BASE_URLS}/api/v1/user/profile/${name}`,
        config
      );
      return data;
    } catch (error) {
  ...
  async (Userformdata, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${process.env.VITE_API_BASE_URLS}/api/v1/user/admin/profile/${_id}`,
        Userformdata,
        config
      );
      return data;
...
export const UpdateProfile = createAsyncThunk(
  ...
  async (profiledata, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${process.env.VITE_API_BASE_URLS}/api/v1/user/profile/${_id}`,
        profiledata,
        config
      );
  ...
  async (name, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.delete(
        `${process.env.VITE_API_BASE_URLS}/api/v1/user/admin/profile/${name}`,
        config
      );
      return name;
    } catch (error) {
