...
// Same base layout as guest-invoice/booking-receipt, different framing:
// a guest invoice shows what the guest paid, this shows a host what they
// actually earned, net of platform fee, that's the number they care
// about, so it's the one emphasized here, not the gross booking total.
...
export function buildHostStatementHtml(data: HostStatementData): string {
  const issueDate = fmtDate(new Date());
  const releasedDate = fmtDate(data.releasedAt);

  const toBlock = buildAddressBlock("Payout to", [data.hostName]);
  const forBlock = buildAddressBlock("For booking", [data.propertyName, `Guest: ${data.guestName}`]);

  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
<title>Payout Statement ${escapeHtml(data.statementNumber)}</title>
<style>${buildStyles()}</style>
</head>
<body>
  <div class="page">
    <div class="header-row">
      <h1 class="doc-title">Payout Statement</h1>
      <div class="logo">${LOGO_SVG}</div>
    </div>

    <div class="meta-row">
      <div class="meta-item"><span class="meta-label">Statement number</span><span class="meta-value">${escapeHtml(data.statementNumber)}</span></div>
    ...
    <div class="due-row">
      <div class="due-amount payout">${fmtNaira(data.hostPayoutNgn)} released on ${releasedDate}</div>
    </div>
    ...
    <table class="items-table">
      ...
      <tbody>
        <tr class="item-row">
          <td class="item-desc">
            <div class="item-title">${escapeHtml(data.roomTypeName)}</div>
            <div class="item-sub">${fmtDate(data.checkIn)} &ndash; ${fmtDate(data.checkOut)}</div>
          </td>
    ...
    <div class="totals-block">
      <div class="totals-row"><span class="totals-label">Booking total</span><span class="totals-value">${fmtNaira(data.totalAmountNgn)}</span></div>
      <div class="totals-row totals-row--deduction"><span class="totals-label">Platform fee</span><span class="totals-value">&minus; ${fmtNaira(data.platformFeeNgn)}</span></div>
      <div class="totals-row totals-row--total"><span class="totals-label">Your payout</span><span class="totals-value">${fmtNaira(data.hostPayoutNgn)}</span></div>
    </div>

    <div class="tx-block">
      <div class="tx-label">Payout details</div>
      <div class="tx-row"><span class="tx-key">Released</span><span class="tx-val">${releasedDate}</span></div>
      <div class="tx-row"><span class="tx-key">Booking reference</span><span class="tx-val">${escapeHtml(data.bookingRef)}</span></div>
    </div>

    <div class="footer">
      This statement reflects escrow released to you for this booking.<br/>
...
.due-amount.payout { color: #166534; }
...
.th-qty, .th-amount { text-align: right; }
...
.totals-row--deduction { color: #991b1b; }
.totals-row--total { border-top: 1px solid #eaeaea; margin-top: 4px; padding-top: 10px; font-weight: 700; color: #166534; font-size: 16px; }
