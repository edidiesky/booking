...
import { query, outboxRepository, queryOne } from "@booking/shared";
...
export const auditRepository = {
  async log(data: {
    ...
    try {
      await outboxRepository.createStandalone("audit.log.requested", {
        ...
        userAgent: data.req
          ? (data.req.headers["user-agent"] as string | undefined)
          : undefined,
        requestId: data.req
          ? ((data.req.headers["x-request-id"] as string | undefined) ??
            ctx?.requestId)
          : ctx?.requestId,
      });
    } catch (err) {
      logger.error("audit_log_enqueue_failed", {
        event: "audit_log_enqueue_failed",
  ...
  > {
    ...
    if (filters?.search) {
      ...
      conditions.push(
        `(a.resource ILIKE $${params.length} OR u.first_name ILIKE $${params.length} OR u.last_name ILIKE $${params.length})`,
      );
    }
    if (filters?.dateFrom) {
...
async listAll(page = 1, limit = 30): Promise<AuditLogEntry[]> {
  const offset = (page - 1) * limit;
  return query<AuditLogEntry>(
