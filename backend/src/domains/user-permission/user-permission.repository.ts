...
export const userPermissionRepository = {
  async upsert(data: {
    ...
    try {
        ...
        `INSERT INTO user_permissions (user_id, tenant_id, permission_id, granted, assigned_by, reason)
         ...
         ON CONFLICT (user_id, tenant_id, permission_id) DO UPDATE
           SET granted     = EXCLUDED.granted,
               ...
               updated_at  = now()
         RETURNING *`,
        [data.userId, data.tenantId, data.permissionId, data.granted, data.assignedBy, data.reason ?? null]
      );
      ...
      trackError("user_permission_upsert_failed", "user_permission_repository", "medium");
      throw err;
    }
  },

  async findByUserAndTenant(userId: string, tenantId: string): Promise<UserPermission[]> {
  ...
  async revoke(userId: string, tenantId: string, permissionId: string): Promise<void> {
    await query(
  ...
  async getPermissionIdsByUser(userId: string, tenantId: string): Promise<{ permission_id: string; granted: boolean }[]> {
    return query<{ permission_id: string; granted: boolean }>(
