...
export const bookingRepository = {
  async create(data: {
    ...
    metadata?:       Record<string, unknown>;
  }, client: PoolClient): Promise<Booking> {
    ...
    try {
      const row = (await client.query(
        `INSERT INTO bookings
         ...
         RETURNING *`,
        [
          bookingRef, data.tenantId, data.propertyId, data.roomTypeId, data.guestUserId,
          data.roomsCount, data.checkIn, data.checkOut, data.guestCount,
          data.totalAmountNgn, data.platformFeeNgn, data.hostPayoutNgn,
          data.specialRequests ?? null, JSON.stringify(data.metadata ?? {}),
        ]
      )).rows[0] as Booking;
  ...
  async findById(id: string): Promise<Booking | null> {
    try {
      return await queryOne<Booking>(`SELECT * FROM bookings WHERE id = $1`, [id]);
    } catch (err) {
      trackError("booking_find_failed", "booking_repository", "medium");
      logger.error("booking_repository_find_by_id_failed", { event: "booking_repository_find_by_id_failed", id, error: (err as Error).message, ...ctx() });
      throw err;
    }
  },
  ...
  async findByRef(ref: string): Promise<Booking | null> {
    try {
      return await queryOne<Booking>(`SELECT * FROM bookings WHERE booking_ref = $1`, [ref]);
    } catch (err) {
      trackError("booking_find_failed", "booking_repository", "medium");
      throw err;
    }
  ...
  async updateStatus(
    ...
    if (extra?.cancellation_reason) { fields.push(`cancellation_reason = $${idx++}`); values.push(extra.cancellation_reason); }
    ...
    try {
      const row = client
        ? (await client.query(sql, values)).rows[0] as Booking | null ?? null
  ...
  async listByGuest(guestUserId: string, page = 1, limit = 20): Promise<Booking[]> {
  ...
  async listByTenant(tenantId: string, opts: { status?: BookingStatus; page?: number; limit?: number } = {}): Promise<Booking[]> {
  ...
  async countByTenant(tenantId: string, status?: BookingStatus): Promise<number> {
    const params: unknown[] = [tenantId];
    const where = status ? `AND status = $${params.push(status)}` : "";
    try {
      const row = await queryOne<{ count: string }>(
        `SELECT COUNT(*) AS count FROM bookings WHERE tenant_id = $1 ${where}`, params
      );
