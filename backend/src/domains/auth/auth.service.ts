...
const twoFactorChallengeKey = (token: string) => `2fa-challenge:${token}`;
  ...
  async confirmEmail(input: ConfirmEmailInput): Promise<void> {
    ...
    if (Date.now() > (state.tokenExpiresAt ?? 0)) {
      throw AppError.badRequest(
        "OTP has expired. Please restart the registration.",
      );
    }
  ...
  async registerHost(input: RegisterHostInput): Promise<AuthTokens> {
    ...
    if (await tenantRepository.slugExists(input.tenantSlug)) {
      throw AppError.conflict(
        "This business URL is already taken. Please choose another.",
      );
    }
      ...
      const tenant = await tenantRepository.create(
        {
          ...
          ownerUserId: userId,
          platformFeePct: input.platformFeePct ?? 10.0,
        },
        client,
      );
      tenantId = tenant.id;
...
async logout(userId: string, accessToken: string, refreshToken?: string): Promise<void> {
  ...
  async resendOtp(email: string): Promise<{ message: string; debug?: string }> {
    ...
    if (!raw)
      throw AppError.badRequest(
        "No onboarding session found. Please restart registration.",
      );
  ...
  async login(
    ...
    if (user.two_factor_enabled) {
      ...
      await redisClient.set(
        twoFactorChallengeKey(challengeToken),
        user.id,
        "EX",
  ...
  async verifyTwoFactorLogin(
    ...
    if (!isValid && user.two_factor_backup_codes?.length) {
      for (const hashed of user.two_factor_backup_codes) {
        if (await bcrypt.compare(code, hashed)) {
          isValid = true;
          consumedBackupCode = true;
          const remaining = user.two_factor_backup_codes.filter(
            (c) => c !== hashed,
          );
    ...
    logger.info("user_logged_in_2fa", {
      ...
      userId: user.id,
      viaBackupCode: consumedBackupCode,
    });
  ...
  async disableTwoFactor(userId: string, password: string) {
    ...
    await auditRepository.log({
      action: "updated",
      resource: "two_factor_disabled",
      resourceId: userId,
      userId,
    });
    logger.info("two_factor_disabled", {
  ...
  async verifyAndEnableTwoFactor(userId: string, token: string) {
    ...
    if (!user) throw AppError.notFound("User.");
    if (!user.two_factor_secret)
      throw AppError.badRequest("Run setup before verifying.");
    if (user.two_factor_enabled)
  ...
  async setupTwoFactor(userId: string) {
    ...
    await userRepository.updateById(userId, { two_factor_secret: secret });
  ...
  async loginWithGoogle(
    ...
    if (!tokens.id_token)
      throw AppError.badRequest("Google did not return an identity token.");
    ...
    if (!payload?.email || !payload.sub)
      throw AppError.badRequest("Could not read your Google profile.");
    if (!payload.email_verified)
      throw AppError.forbidden("Your Google email is not verified.");
    ...
    if (!user) {
      ...
      if (existingByEmail) {
        // Auto-link: Google has already verified this email, safe to
        // attach the Google identity to the existing password account.
        await userRepository.updateById(existingByEmail.id, {
          google_id: payload.sub,
          is_email_verified: true,
        });
        user = { ...existingByEmail, google_id: payload.sub };
      } else {
        // Brand new signup via Google, defaults to guest, matching the
        // same default a plain email signup would get without an
        // explicit host onboarding flow. password_hash is a random,
        // never-revealed value, this account can only ever log in via
        // Google unless a separate "set a password" flow is added later.
        const randomPassword = await bcrypt.hash(nanoid(32), 12);
        ...
        await userRepository.updateById(created.id, {
          google_id: payload.sub,
          is_email_verified: true,
        });
        user = await userRepository.findByIdWithSecrets(created.id);
        if (!user)
          throw AppError.internal("User creation failed unexpectedly.");
      }
    }
