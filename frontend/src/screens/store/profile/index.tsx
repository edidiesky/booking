    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center flex flex-col gap-4">
          <p className="text-xs text-[#666]">You need to be signed in to view your profile.</p>
          <button
            onClick={() => navigate("/login")}
            className="h-10 px-6 bg-[#171717] text-white text-xs  hover:opacity-90 transition-opacity"
          >
            Sign in
          </button>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 lg:px-8 py-12 flex flex-col gap-8">
        ...
        <div className="flex items-center gap-4">
          {user?.profileImage ? (
            ...
            <div className="w-14 h-14 bg-[#171717] flex items-center justify-center text-white text-lg ">
              {currentUser.firstName.charAt(0).toUpperCase()}
            </div>
          )}
          <div>
            <p className="text-base  text-[#171717]">{currentUser.firstName} {currentUser.lastName}</p>
        ...
        {activeTab === "profile" && (
          <form onSubmit={profileForm.handleSubmit(onProfileSubmit)} className="flex flex-col gap-5 max-w-lg">
              ...
              {(["firstName", "lastName"] as const).map((field) => (
                <label key={field} className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    className="h-[42px] border border-black/10 px-4 text-xs outline-none focus:border-[#171717] transition-colors"
                  />
            ...
            <label className="flex flex-col gap-1.5">
              ...
              <input {...profileForm.register("phone")} placeholder="+234..." className="h-[42px] border border-black/10 px-4 text-xs outline-none focus:border-[#171717] transition-colors" />
            </label>
            <label className="flex flex-col gap-1.5">
              <span className="text-xs  text-[#171717]">Gender</span>
              <select {...profileForm.register("gender")} className="h-[42px] border border-black/10 px-4 text-xs bg-white outline-none focus:border-[#171717] transition-colors">
                <option value="">Select gender</option>
                <option value="Male">Male</option>
                <option value="Female">Female</option>
              </select>
            </label>
            <button type="submit" disabled={updatingProfile} className="h-11 bg-[#171717] text-white text-xs  hover:opacity-90 disabled:opacity-50 w-fit px-6">
        ...
        {activeTab === "orders" && (
          <div className="flex flex-col gap-4">
            {ordersLoading ? (
              <p className="text-xs text-[#666]">Loading orders...</p>
            ) : orders.length === 0 ? (
              <div className="text-center py-16 flex flex-col gap-3">
                <p className="text-xs text-[#666]">You have no orders yet.</p>
                <button onClick={() => navigate(`/store/${storeId}`)} className="text-xs underline underline-offset-4 text-[#171717]">
                  Start shopping
                </button>
              </div>
            ) : (
                    ...
                    return (
                      <div key={order._id} className="bg-white border border-black/5 p-5 flex flex-col gap-3">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className={`text-xs  px-2 py-0.5 ${osCfg.className}`}>{osCfg.label}</span>
                          <span className={`text-xs  px-2 py-0.5 ${fsCfg.className}`}>{fsCfg.label}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <p className="text-xs text-[#666]">{order.items.length} item{order.items.length !== 1 ? "s" : ""}</p>
                          <p className="text-xs  text-[#171717]">₦{order.totalAmount.toLocaleString("en-NG")}</p>
                        </div>
          ...
          <form onSubmit={passwordForm.handleSubmit(onPasswordSubmit)} className="flex flex-col gap-5 max-w-lg">
            {(["currentPassword", "newPassword", "confirmPassword"] as const).map((field) => (
              <label key={field} className="flex flex-col gap-1.5">
                ...
                <input type="password" {...passwordForm.register(field)} className="h-[42px] border border-black/10 px-4 text-xs outline-none focus:border-[#171717] transition-colors" />
                {passwordForm.formState.errors[field] && (
                  <span className="text-xs text-red-600">{passwordForm.formState.errors[field]?.message}</span>
                )}
              </label>
            ))}
            <button type="submit" disabled={changingPassword} className="h-11 bg-[#171717] text-white text-xs  hover:opacity-90 disabled:opacity-50 w-fit px-6">
        ...
        {activeTab === "address" && (
          <form onSubmit={addressForm.handleSubmit(onAddressSubmit)} className="flex flex-col gap-5 max-w-lg">
            <label className="flex flex-col gap-1.5">
              ...
              <input {...addressForm.register("street")} placeholder="123 Main Street" className="h-[42px] border border-black/10 px-4 text-xs outline-none focus:border-[#171717] transition-colors" />
            ...
            <div className="grid grid-cols-2 gap-4">
              {(["city", "state"] as const).map((field) => (
                <label key={field} className="flex flex-col gap-1.5">
                  ...
                  <input {...addressForm.register(field)} className="h-[42px] border border-black/10 px-4 text-xs outline-none focus:border-[#171717] transition-colors" />
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className="flex flex-col gap-1.5">
                ...
                <input {...addressForm.register("country")} placeholder="Nigeria" className="h-[42px] border border-black/10 px-4 text-xs outline-none focus:border-[#171717] transition-colors" />
              ...
              <label className="flex flex-col gap-1.5">
                ...
                <input {...addressForm.register("postalCode")} placeholder="100001" className="h-[42px] border border-black/10 px-4 text-xs outline-none focus:border-[#171717] transition-colors" />
              </label>
            </div>
            <button type="submit" className="h-11 bg-[#171717] text-white text-xs  hover:opacity-90 w-fit px-6">
              Save address
            </button>
          </form>
        )}
