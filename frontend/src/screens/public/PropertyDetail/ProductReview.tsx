...
//         <button
//         onClick={onViewAll}
//         className="border rounded-full p-4 py-2 text-xs lg:text-[13px]   border-[#000] bold text-[#171717]"
//       >
...
import { useGetRoomTypeReviewsQuery } from "@/redux/services/reviewApi";
import ProductReviewSummary from "./ProductReviewSummary";
import ProductReviewList from "./ProductReviewList";
...
export default function ProductReview({ roomTypeId }: Props) {
  const [showAll, setShowAll] = useState(false);
  const { data, isLoading } = useGetRoomTypeReviewsQuery(
    { roomTypeId, limit: showAll ? 50 : 10 },
    { skip: !roomTypeId },
  );
  ...
  return (
    <div className="w-full flex flex-col gap-6 lg:gap-14">
      <ProductReviewSummary stats={stats} />
      <ProductReviewList reviews={reviews} limit={showAll ? undefined : 3} />
      {!showAll && stats.totalReviews > 3 && (
        <div className="w-full flex items-center justify-center">
          <button
            ...
            className="border rounded-full p-4 py-2 text-xs lg:text-[13px]   border-[#000] bold text-[#171717]"
          >
            View all reviews for this room type
          </button>
        </div>
      )}
    </div>
