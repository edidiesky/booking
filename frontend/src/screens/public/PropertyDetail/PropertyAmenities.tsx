import { Wifi, Wind, Tv, Car, Zap, Shield, Coffee, Waves, Dumbbell, Utensils } from "lucide-react";
...
export default function PropertyAmenities({ property }: Props) {
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      <h3 className="text-2xl bold md:text-3xl  text-[#17191c]">
        Room Services
        <span className="block text-base text-[#777b86] pt-1">
          Enjoy the comforts of home and beyond with these distinctive features.
        </span>
      </h3>
      <div className="w-full grid grid-cols-2 md:grid-cols-4 gap-x-3 gap-y-6">
        {property.amenities.map((amenity, i) => (
          <div key={i} className="flex items-center gap-3 text-base bold text-[#4c4c4c]">
