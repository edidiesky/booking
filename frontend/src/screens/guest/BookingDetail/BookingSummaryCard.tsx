    ...
    <div className="rounded-xl bg-white border overflow-hidden flex flex-col w-full">
      ...
      <div className="w-full flex flex-col gap-2">
        ...
        <div className="w-full flex p-4 pb-6 px-8 border-b-4 flex-col gap-2">
          <div className="w-full text-xs lg:text-sm flex items-center justify-between text-[#17191c]">
            <span>{booking.nights} night{booking.nights !== 1 ? "s" : ""}</span>
            <span>{formatCurrency(booking.totalAmountNgn - booking.platformFeeNgn)}</span>
          </div>
          <div className="w-full text-xs lg:text-smpb-4 bold flex items-center justify-between text-[#17191c]">
            <span>Platform fee</span>
            <span>{formatCurrency(booking.platformFeeNgn)}</span>
          </div>
          <div className="w-full text-xs lg:text-sm pt-4 border-t flex items-center justify-between text-[#17191c]">
            <span>Total</span>
