        ...
        <div className="flex p-8 pb-6 border-b-4 flex-col w-full gap-2">
          ...
          <div className="py-3 flex items-center justify-between w-full">
            <div className="flex flex-col gap-2">
              <span className="text-xs lg:text-[13px]  text-[#17191c]">Dates</span>
              <span className="text-xs lg:text-[13px]  text-[#4c4c4c]">{checkInLabel} - {checkOutLabel}</span>
            </div>
            <div className="flex flex-col gap-2">
              <span className="text-xs lg:text-[13px]  text-[#17191c]">Guests</span>
              <span className="text-xs lg:text-[13px]  text-[#4c4c4c]">{booking.guestCount} guest{booking.guestCount !== 1 ? "s" : ""}</span>
            </div>
          </div>
        </div>

        <div className="w-full flex p-4 pb-6 px-8 border-b-4 flex-col gap-2">
          <div className="w-full text-xs lg:text-[13px]  flex items-center justify-between text-[#17191c]">
            <span>{booking.nights} night{booking.nights !== 1 ? "s" : ""}</span>
            <span>{formatCurrency(booking.totalAmountNgn - booking.platformFeeNgn)}</span>
          </div>
          <div className="w-full text-xs lg:text-[13px]pb-4 bold flex items-center justify-between text-[#17191c]">
            <span>Platform fee</span>
            <span>{formatCurrency(booking.platformFeeNgn)}</span>
          </div>
          <div className="w-full text-xs lg:text-[13px]  pt-4 border-t flex items-center justify-between text-[#17191c]">
            <span>Total</span>
