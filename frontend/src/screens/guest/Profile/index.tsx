      ...
      <div className="max-w-screen-2xl w-full lg:w-[90%] mx-auto lg:px-12 flex items-start justify-start flex-col gap-4">
        <div className="w-full flex items-center gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <span className="text-xs lg:text-xs capitalize text-[#777b86] truncate">{`${user?.userType} profile`}</span>
          </div>
        </div>
            ...
            <div className="flex gap-2 flex-col items-start">
              {TABS.map((t) => (
                <button
                  ...
                  className={`px-4 min-w-56 lg:min-w-48 text-start  hover:bg-[#f5f5f3] py-3 text-xs lg:text-xs ${active === t.key ? "border-r-2 bg-[#f5f5f3] border-[#17191c] font-semibold" : "text-[#a3a6af]"}`}
                >
                  {t.label}
                </button>
              ))}
