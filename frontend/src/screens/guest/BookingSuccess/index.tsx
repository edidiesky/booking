    ...
    <div className="min-h-screen bg-[#f7f7f8] flex items-center justify-center px-4 py-12">
      ...
      >
        ...
        {!isLoading && booking && (
          <>
              ...
              {STEPS.map((step, i) => (
                <div key={step.key} className="flex items-center flex-1">
                  <div className="flex flex-col items-center gap-1.5 flex-1">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs lg:text-[13px] ${
                      i <= stepIdx ? "bg-[#17191c] text-white" : "border-2 border-[#e8e6e3] text-[#a3a6af]"
                    }`}>
                      {i < stepIdx ? "✓" : i + 1}
                    </div>
                    <span className={`text-xs lg:text-[13px]${i <= stepIdx ? "text-[#17191c] bold" : "text-[#a3a6af]"}`}>{step.label}</span>
                  </div>
            ...
            <button
              ...
              className="w-full h-12 rounded-full border border-[#e8e6e3] text-xs lg:text-[13px] text-[#17191c] hover:bg-[#f2f0ed] transition-colors"
            >
              View your booking
            </button>
            ...
            <div className="border border-[#e8e6e3] rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-[#e8e6e3]">
                <p className="text-xs lg:text-[13px] uppercase medium text-[#777b86]">Booking details</p>
                <p className="text-xs lg:text-[13px] mt-0.5">Confirmation: {booking.bookingRef}</p>
              </div>
              ...
              <div className="px-4 py-4 flex gap-3 border-b items-center border-[#f2f0ed]">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs lg:text-[13px]lg:text-lg bold text-[#17191c] truncate">{booking.propertyName}</p>
                  <p className="text-xs lg:text-[13px] text-[#777b86]">{booking.nights} night{booking.nights !== 1 ? "s" : ""} · {booking.guestCount} guest{booking.guestCount !== 1 ? "s" : ""}</p>
                </div>
                <p className="text-xs lg:text-[13px] text-[#17191c] whitespace-nowrap">{formatCurrency(booking.totalAmountNgn)}</p>
              </div>

              <div className="px-4 py-4 grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs lg:text-[13px] uppercase text-[#a3a6af] bold mb-1">Check-in</p>
                  <p className="text-xs lg:text-[13px] text-[#17191c]">{formatDate(booking.checkIn)}</p>
                </div>
                <div>
                  <p className="text-xs lg:text-[13px] uppercase text-[#a3a6af] bold mb-1">Check-out</p>
                  <p className="text-xs lg:text-[13px] text-[#17191c]">{formatDate(booking.checkOut)}</p>
                </div>
              </div>

              <div className="px-4 py-3 bg-[#f7f7f8] flex items-center justify-between">
                <span className="text-xs lg:text-[13px] text-[#17191c]">Total (1 booking)</span>
                <span className="text-xs lg:text-[13px] text-[#17191c]">{formatCurrency(booking.totalAmountNgn)}</span>
              </div>
            </div>

            {booking.receiptUrl && (
              <a href={booking.receiptUrl} target="_blank" rel="noopener noreferrer"
                className="text-xs lg:text-[13px] text-center underline underline-offset-4 text-[#777b86] hover:text-[#17191c] transition-colors">
                View receipt
              </a>
            )}
            ...
            {booking.propertyName && (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-[#17191c] text-white flex items-center justify-center text-xs lg:text-[13px] shrink-0">
                    {booking.propertyName.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <p className="text-xs lg:text-[13px] text-[#17191c]">{booking.propertyName}</p>
                    <p className="text-xs lg:text-[13px] text-[#a3a6af]">{booking.propertyCity}</p>
                  </div>
                </div>
        ...
        <button
          ...
          className="w-full h-14 rounded-full bg-[#17191c] text-white text-xs lg:text-[13px] hover:opacity-90 transition-opacity"
        >
          Continue browsing
        </button>
      </motion.div>
