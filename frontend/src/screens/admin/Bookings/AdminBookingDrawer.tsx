      ...
      <DrawerSection label="Room">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-1">
            ...
            <p className="text-xs lg:text-sm text-[#777b86] truncate">{booking.roomTypeName}</p>
          ...
          <div className="flex flex-col gap-4">
            {TIMELINE_STEPS.map((step, i) => (
              <div key={step.key} className="flex items-start gap-3">
                ...
                <div>
                  <p className={`text-xs lg:text-sm ${i <= idx ? "text-[#17191c]" : "text-[#a3a6af]"}`}>
                    {step.label}
                  </p>
                  <p className="text-xs lg:text-sm text-[#a3a6af]">{step.description}</p>
                </div>
              </div>
            ))}
          </div>
