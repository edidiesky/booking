...
interface Props {
  ...
  onViewDetails: (payment: PaymentSummary & { tenant_name: string }) => void;
}
export default function PaymentTableRow({ payment, onViewDetails }: Props) {
  return (
    ...
    >
      ...
      <td
        className="px-5 py-3 bold whitespace-nowrap"
        style={{ color: "var(--color-ink)" }}
      >
        {formatCurrency(Number(payment.amount_ngn))}
      </td>
      <td
        className="px-5 py-3 capitalize text-xs"
        style={{ color: "var(--color-muted-stone)" }}
      >
        {payment.gateway}
      </td>
