    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-base p-4 flex items-center justify-end z-50">
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e8e6e3]">
          <div>
            <p className="text-xs lg:text-sm text-[#17191c]">Payment details</p>
            <p className="text-xs lg:text-sm text-[#777b86] mt-0.5">{payment?.id}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 flex w-full overflow-y-auto flex-col gap-2">
          <div className="w-full px-6 py-5 border-b flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs lg:text-sm text-[#17191c] truncate">{payment.room_type_name}</p>
                <p className="text-xs lg:text-sm text-[#777b86]">{nights} night{nights !== 1 ? "s" : ""}</p>
              </div>
          ...
          <div className="w-full px-6 py-5 border-b flex flex-col gap-6">
            <p className="w-full text-xs lg:text-sm text-[#a3a6af] uppercase">
              Payment breakdown
            </p>
            <div className="flex flex-col gap-4 lg:gap-4 lg:w-[75%]">
              {[
                ...
                <div key={label} className="flex items-center justify-between">
                  <span className="text-xs lg:text-sm text-[#777b86]">{label}</span>
                  <span className="text-xs lg:text-sm text-[#17191c] bold text-right max-w-[60%] truncate">
                    {value}
                  </span>
                </div>
              ))}
          ...
          <div className="w-full px-6 py-5 border-b flex flex-col gap-4">
            ...
            {[
              ...
              >
                ...
                <p className="text-xs lg:text-sm text-[#17191c] bold">{value}</p>
              </div>
            ))}
          </div>
            ...
            <div className="flex flex-col gap-4">
              {TIMELINE_STEPS.map((step, i) => (
                <div key={step.key} className="flex items-start gap-3">
                  ...
                  <div>
                    <p className={`text-xs lg:text-sm ${i <= idx ? "text-[#17191c]" : "text-[#a3a6af]"}`}>{step.label}</p>
                    <p className="text-xs lg:text-sm medium text-[#a3a6af]">{step.description}</p>
                  </div>
                </div>
              ))}
            </div>
