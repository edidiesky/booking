...
function ImageSection({ images, onChange }: ImageSectionProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <span className="text-xs lg:text-sm text-[#17191c]">Room Photos</span>
      ...
      {hasAny ? (
        ...
        >
          ...
          <p className="text-xs lg:text-sm text-[#a3a6af]">PNG, JPG, WebP</p>
        </div>
      )}
...
function TagInput({ label, placeholder, tags, onChange }: TagInputProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <span className="text-xs lg:text-sm text-[#17191c]">{label}</span>
      <div className=" py-2 px-2 flex flex-wrap gap-2 min-h-[45px] focus-within:border-[#17191c] transition-colors">
        {tags.map((tag, i) => (
          <span
            key={i}
            className="inline-flex items-center gap-1 px-4 rounded-full bold py-1 bg-[#f2f0ed] text-xs lg:text-sm text-[#17191c]"
          >
            {tag}
            <button
              type="button"
      ...
      <p className="text-xs lg:text-sm text-[#a3a6af]">Press Enter or comma to add</p>
    </div>
  );
}
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm p-4 flex items-center justify-end z-50">
      ...
      >
        ...
        <div className="border-b border-[#e8e6e3] flex items-center justify-between px-8 h-[72px] shrink-0">
          <div>
            <h4 className="text-xs lg:text-sm text-[#17191c]">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs lg:text-sm text-[#17191c]">Status</label>
                <Select
                  value={form.status}
                  onValueChange={(v) =>
                ...
                >
                  <SelectTrigger className="h-[42px] border border-[#e8e6e3] px-3 text-xs lg:text-smoutline-none focus:border-[#17191c] transition-colors bg-white text-[#17191c] w-full rounded-none shadow-none focus:ring-0">
                  ...
                  <SelectContent className="bg-white border border-[#e8e6e3] shadow-sm rounded-xl">
                    {[
                      ...
                      <SelectItem
                        key={opt.value}
                        value={opt.value}
                        className="text-xs lg:text-sm text-[#17191c] cursor-pointer hover:bg-[#f2f0ed] focus:bg-[#f2f0ed] focus:text-[#17191c]"
                      >
                        {opt.label}
                      </SelectItem>
                    ))}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs lg:text-sm text-[#17191c]">Description</label>
                <textarea
                  value={form.description}
                  onChange={(e) => setField("description", e.target.value)}
                  rows={4}
                  className="border border-[#e8e6e3] px-3 py-2.5 text-xs lg:text-smoutline-none resize-none focus:border-[#17191c] transition-colors"
        ...
        <div className="border-t border-[#e8e6e3] bg-white h-[68px] flex items-center justify-between px-8 shrink-0">
          ...
          <button
            onClick={handleSave}
            disabled={isBusy}
            className="bg-[#17191c] text-white text-xs lg:text-smrounded-full px-6 h-9 flex items-center gap-2 hover:opacity-90 disabled:opacity-50 transition-opacity"
          >
