...
import type { Property, TenantInfo } from "@/types/api";
interface Props {
  property: Property & TenantInfo;
  onClose: () => void;
}
  ...
  return (
    <Drawer title={property.name} subtitle={property.tenantName} onClose={onClose} widthClass="lg:w-[640px]">
      <DrawerSection label="Property">
        <div className="flex items-center gap-3">
          <div className="w-16 h-16 rounded-lg overflow-hidden bg-[#f2f0ed] shrink-0">
            {property.images?.[0] ? <LazyImage src={property.images[0]} alt={property.name} /> : null}
          </div>
      ...
      <DrawerSection label="Seller / Tenant">
        <DrawerField label="Name"          value={property.tenantName} />
        <DrawerField label="Tenant ID"     value={property.tenantId} />
        <DrawerField label="Contact Email" value={property.tenantEmail} />
      </DrawerSection>

      <div className="border-t sticky bottom-0 left-0 bg-white border-[#e8e6e3] px-6 py-4 flex items-center justify-between">
        <button onClick={onClose} className="text-xs text-[#777b86] bold hover:text-[#17191c]">Close</button>
      </div>
    </Drawer>
  );
}