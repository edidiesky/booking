    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm p-4 flex items-center justify-end z-50">
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e8e6e3]">
          <p className="text-xs lg:text-[13px] text-[#17191c]">{roomType?.name ?? "Room type"}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-6">
              ...
              <div className="flex flex-col gap-6">
                ...
                <div>
                  <div className="flex items-center justify-between">
                    <p className="text-xs lg:text-[13px] text-[#17191c]">{roomType.name}</p>
                    <span className={`text-xs lg:text-[13px]px-2 py-0.5 rounded-full bold ${(STATUS_CFG[roomType.status] ?? STATUS_CFG.inactive).className}`}>
                      {(STATUS_CFG[roomType.status] ?? STATUS_CFG.inactive).label}
                    </span>
                  </div>
                  <p className="text-xs lg:text-[13px] text-[#777b86] mt-2 leading-relaxed">{roomType.description ?? "No description provided."}</p>
                </div>

                {roomType.amenities?.length > 0 && (
                  <div>
                    <p className="text-xs lg:text-[13px] uppercase text-[#a3a6af] bold mb-2">Amenities</p>
                    <div className="flex flex-wrap gap-2">
                      {roomType.amenities.map((a) => (
                        <span key={a} className="text-xs lg:text-[13px]px-3 py-1 rounded-full bg-[#f2f0ed] text-[#4c4c4c]">{a}</span>
                      ))}
                    </div>
                  </div>
                )}
              ...
              <div className="flex flex-col gap-4">
                <div className="border border-[#e8e6e3] rounded-xl p-4 flex flex-col gap-3">
                  <p className="text-xs lg:text-[13px] uppercase text-[#a3a6af] bold">Pricing & capacity</p>
                ...
                <div className="border border-[#e8e6e3] rounded-xl p-4 flex flex-col gap-3">
                  <p className="text-xs lg:text-[13px] uppercase text-[#a3a6af] bold">Current occupant</p>
                  {occupant ? (
                    <>
                      <div className="flex items-center justify-between text-xs">
                    ...
                    <p className="text-xs lg:text-[13px] text-[#a3a6af]">Vacant, no active booking.</p>
                  )}
                </div>
              </div>
            </div>
