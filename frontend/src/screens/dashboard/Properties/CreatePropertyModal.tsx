    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center"
      ...
      <div className="w-full max-w-lg rounded-2xl p-8 flex flex-col gap-6 overflow-y-auto max-h-[90vh]"
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold" style={{ color: "var(--color-ink)" }}>Add Property</h2>
        ...
        <form onSubmit={handleSubmit(submit)} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input {...register("name")} className={field} style={fieldStyle} placeholder="e.g. Lekki Heights Shortlet" />
            {errors.name && <p className="text-xs text-red-500">{errors.name.message}</p>}
          </div>

          {/* Type */}
          <div className="flex flex-col gap-1">
            <label className="text-xs" style={{ color: "var(--color-muted-stone)" }}>Property Type</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            {([ ["street", "Street", "5 Admiralty Way"], ["city", "City", "Lagos"],
                 ["state", "State", "Lagos"],            ["country", "Country", "Nigeria"] ] as const).map(([key, label, ph]) => (
              <div key={key} className="flex flex-col gap-1">
                <label className="text-xs" style={{ color: "var(--color-muted-stone)" }}>{label}</label>
                <input {...register(key)} className={field} style={fieldStyle} placeholder={ph} />
