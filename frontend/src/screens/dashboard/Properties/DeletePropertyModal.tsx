    ...
    <div className="h-[100vh] bg-[#16161639] inset-0 backdrop-blur-sm w-full fixed top-0 left-0 z-[5000] flex items-end md:items-center justify-end md:justify-center px-4">
      ...
      >
        <div className="w-full flex px-8 items-start justify-between gap-1">
          <div>
            <h3 className="text-xl text-[#17191c]">Delete Property</h3>
            <p className="text-sm text-[#777b86] mt-1 max-w-[380px]">
        ...
        <div className="w-full flex px-8 py-4 border-t border-[#e8e6e3] items-center justify-end gap-3">
          <button
            ...
            onClick={onClose}
            disabled={isLoading}
            className="h-9 px-5 text-sm bold rounded-full text-[#4c4c4c] border border-[#e8e6e3] hover:bg-[#f2f0ed] transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            ...
            disabled={isLoading}
            className="h-9 px-5 text-sm bold rounded-full bg-red-600 text-white hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center gap-2"
          >
            {isLoading ? (
              <>
