    ...
    <div className="flex flex-col gap-3">
      ...
      {hasAny ? (
        ...
        >
          ...
          <p className="text-xs lg:text-smtext-[#a3a6af]">PNG, JPG, WebP</p>
        </div>
      )}
...
function TagInput({ label, placeholder, tags, onChange }: TagInputProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <span className="text-xs lg:text-smtext-[#17191c]">{label}</span>
      <div className=" py-2 px-2 flex flex-wrap gap-2 min-h-[45px] focus-within:border-[#17191c] transition-colors">
        {tags.map((tag, i) => (
          <span
            key={i}
            className="inline-flex items-center gap-1 px-4 rounded-full bold py-1 bg-[#f2f0ed] text-xs lg:text-smtext-[#17191c]"
          >
            {tag}
            <button
              type="button"
      ...
      <p className="text-xs lg:text-smtext-[#a3a6af]">Press Enter or comma to add</p>
    </div>
  );
}
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6">
          <AnimatePresence mode="wait">
            ...
            >
                  ...
                  <SelectContent className="bg-white border border-[#e8e6e3] shadow-sm rounded-xl">
                    {[
                      ...
                      <SelectItem
                        key={opt.value}
                        value={opt.value}
                        className="text-xs lg:text-smtext-[#17191c] cursor-pointer hover:bg-[#f2f0ed] focus:bg-[#f2f0ed] focus:text-[#17191c]"
                      >
                        {opt.label}
                      </SelectItem>
                    ))}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs lg:text-smtext-[#17191c]">Description</label>
                <textarea
