  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4">
        <Title
          title="Roles & Access"
          description="Manage which team members can access your dashboard and what they can do."
        />
        <button
      ...
      <div className="flex flex-wrap gap-3">
        {roles
          .filter((r) => !["platform:admin", "guest"].includes(r.slug))
          .map((r) => {
            const cfg = ROLE_COLORS[r.slug] ?? { bg: "#f2f0ed", color: "#4c4c4c" };
            return (
              <div key={r.id} className="flex flex-col gap-0.5 px-4 py-3 rounded-xl border"
                   style={{ borderColor: "var(--color-fog)" }}>
                <span className="text-xs px-2 py-0.5 rounded-full font-medium"
                      style={{ backgroundColor: cfg.bg, color: cfg.color }}>
                  {r.name}
                </span>
                <p className="text-xs mt-1" style={{ color: "var(--color-muted-stone)" }}>
                  {r.description}
                </p>
              </div>
            );
