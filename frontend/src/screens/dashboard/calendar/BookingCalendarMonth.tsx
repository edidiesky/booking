      ...
      <div className="border border-[#e8e6e3] rounded-xl overflow-hidden bg-white">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#e8e6e3]">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="flex items-center gap-1.5 h-9 px-4 bg-[#17191c] text-white text-xs rounded-full hover:opacity-90 transition-opacity"
            >
              Add booking
            </button>
          </div>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-[#e8e6e3]">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs font-semibold text-[#17191c] ml-1">
              {format(monthCursor, "MMMM yyyy")}
            </p>
          </div>
          <button
            onClick={() => setMonthCursor(new Date())}
            className="h-8 px-4 border border-[#e8e6e3] rounded-full text-xs text-[#17191c] hover:bg-[#f2f0ed] transition-colors"
          >
            Today
          </button>
        </div>
        ...
        <div className="grid grid-cols-7 border-b border-[#f2f0ed]">
          {WEEKDAYS.map((d) => (
            <div
              key={d}
              className="text-center text-xs text-[#a3a6af] uppercase py-2 border-r border-[#f2f0ed] last:border-r-0"
            >
              {d}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7">
          ...
          {days.map((day) => {
            ...
            return (
              ...
              >
                <span
                  className={`text-xs px-1 w-fit rounded-full ${today ? "bg-[#17191c] text-white font-semibold" : "text-[#777b86]"}`}
                >
                  {format(day, "d")}
                </span>
        ...
        <div className="flex flex-wrap gap-4 px-5 py-3 border-t border-[#e8e6e3]">
          {Object.entries(STATUS_MAP).map(([status, style]) => (
            <div key={status} className="flex items-center gap-1.5">
              ...
              />
              <span className="text-xs text-[#777b86]">{style.label}</span>
            </div>
          ))}
        </div>
      </div>
