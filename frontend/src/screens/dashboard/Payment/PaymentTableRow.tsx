...
export default function PaymentTableRow({ payment, onViewDetails }: Props) {
  return (
    ...
    >
      <td className="px-5 py-3 text-xs lg:text-smwhitespace-nowrap" style={{ color: "var(--color-hint-of-grey)" }}>{payment.id?.slice(0, 10)}...</td>
      <td className="px-5 py-3 text-xs lg:text-smwhitespace-nowrap" style={{ color: "var(--color-muted-stone)" }}>{payment.booking_id?.slice(0, 10)}...</td>
      ...
      <td className="px-5 py-3 text-xs lg:text-smwhitespace-nowrap" style={{ color: "var(--color-muted-stone)" }}>{formatDate(payment.created_at)}</td>
