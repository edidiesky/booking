...
export default function BookingTableRow({
  ...
  return (
    <tr onClick={() => setSelectedOrder()} className="border-b cursor-pointer last:border-0 hover:bg-[#fafaf9] transition-colors" style={{ borderColor: "#f2f0ed" }}>
      <td className="px-5 py-3 bold text-xs lg:text-smwhitespace-nowrap" style={{ color: "var(--color-ink)" }}>
        {booking.bookingRef}
      </td>
      <td className="px-5 py-3 text-xs lg:text-smwhitespace-nowrap" style={{ color: "var(--color-muted-stone)" }}>
        {(booking.propertyName)}
      </td>
      <td className="px-5 py-3 text-xs lg:text-smwhitespace-nowrap" style={{ color: "var(--color-muted-stone)" }}>
        {formatDateTime(booking.checkOut)}
      </td>
      <td className="px-5 py-3 text-xs lg:text-smwhitespace-nowrap" style={{ color: "var(--color-ink)" }}>
