...
const initialState: AuthState = {
  ...
  isAuthenticated: !!localStorage.getItem("auth:accessToken"),
  tenantSlug: localStorage.getItem("auth:tenantSlug"),
  onboardingStep: 1,
  ...
  reducers: {
    setCredentials: (
      ...
      try {
        ...
        if (action.payload.tenantSlug) {
          localStorage.setItem("auth:tenantSlug", action.payload.tenantSlug);
        }
      } catch {
    ...
    clearCredentials: (state) => {
      ...
      try {
        ...
        localStorage.removeItem("auth:tenantSlug");
      } catch {
        /* ignore */
      }
    },
