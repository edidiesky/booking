...
import {
  ...
  type JobState,
} from "@/redux/services/roomImportApi";
...
export default function ImportRoomTypesModal({ propertyId, onClose }: Props) {
  ...
  const [fetchJobStatus] = useLazyGetJobStatusQuery();
    ...
    pollTimer.current = setInterval(async () => {
      try {
        const result = await fetchJobStatus({
          jobType: "csv_room_import",
          jobId,
        }).unwrap();
        setJobState(result.data);
        if (result.data.state === "done" || result.data.state === "error") {
          if (pollTimer.current) clearInterval(pollTimer.current);
        }
      } catch {
        // transient poll failure, keep trying, the job itself is still running
      }
    }, POLL_INTERVAL_MS);
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const result = await importRoomTypes({
        propertyId,
  ...
  const isRunning =
    jobState &&
    (jobState.state === "queued" || jobState.state === "processing");
    ...
    <div className="h-[100vh] bg-[#16161639] inset-0 backdrop-blur-sm w-full fixed top-0 left-0 z-[5000] flex items-end md:items-center justify-end md:justify-center px-4">
      ...
      >
        <div className="w-full flex px-8 items-start justify-between gap-1">
          ...
          <button
            onClick={onClose}
            className="p-1 rounded-full hover:bg-[#f2f0ed]"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="w-full px-8 flex flex-col gap-4">
          ...
          {isDone && (
            <div className="flex flex-col gap-2">
              <div
                className="flex items-center gap-2 text-xs lg:text-[13px]"
                style={{ color: "#166534" }}
              >
                <CheckCircle2 size={14} />
                {jobState.succeeded ?? 0} room type
                {jobState.succeeded === 1 ? "" : "s"} imported
                {(jobState.failed ?? 0) > 0 &&
                  `, ${jobState.failed} row${jobState.failed === 1 ? "" : "s"} failed`}
              </div>
              {jobState.errors && jobState.errors.length > 0 && (
                <div
                  className="max-h-40 overflow-y-auto border rounded-lg p-2 flex flex-col gap-1"
