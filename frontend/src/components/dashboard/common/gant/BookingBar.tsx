...
export default function BookingBar({ bar, onClick }: Props) {
  const top = bar.lane * (ROW_HEIGHT_PX + LANE_GAP_PX) + 8;

  return (
    <button
      onClick={onClick}
      title={`${bar.booking.bookingRef} · ${bar.label}`}
      className="absolute rounded-full px-3 flex flex-col justify-center overflow-hidden text-left transition-transform hover:scale-[1.02] hover:z-10"
      style={{
        left: `${bar.leftPct}%`,
        width: `calc(${bar.widthPct}%)`,
        top,
    ...
    >
      ...
      <span className="text-xs bold truncate opacity-90">
