    ...
    <div className="w-full border rounded-xl overflow-hidden" style={{ borderColor: "#e8e6e3" }}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 p-3 border-b" style={{ borderColor: "#e8e6e3" }}>
        <div className="flex items-center gap-1 rounded-full border p-0.5" style={{ borderColor: "#e8e6e3" }}>
          {(["day", "week", "month"] as GanttView[]).map((v) => (
            <button
              key={v}
              onClick={() => setView(v)}
              className="px-3 h-7 text-xs lg:text-sm rounded-full capitalize transition-colors"
        ...
        <div className="flex items-center gap-2">
          <MultiSelectDropdown
            ...
            searchable
          />
          <label className="flex items-center gap-1.5 text-xs lg:text-smpx-2" style={{ color: "#777b86" }}>
            Max rows
            <input
              type="number"
              min={1}
              ...
              className="w-12 h-7 border rounded-full px-2 text-xs lg:text-smoutline-none"
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={goToday} className="h-7 px-3 text-xs lg:text-smrounded-full border hover:bg-[#f2f0ed]" style={{ borderColor: "#e8e6e3", color: "#17191c" }}>
              Today
            </button>
      ...
      <div className="flex">
        ...
        <div className="shrink-0 border-r" style={{ width: ROOM_COL_WIDTH, borderColor: "#e8e6e3" }}>
          <div className="h-9 border-b flex items-center px-3 text-xs lg:text-sm" style={{ borderColor: "#e8e6e3", color: "#a3a6af" }}>
            Room
          </div>
            ...
            {rows.map((row) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="text-xs lg:text-sm truncate" style={{ color: "#17191c" }}>{row.roomTypeName}</p>
            ...
            {rows.length === 0 ? (
              bookings.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-2 p-16 text-center" style={{ width: totalWidth }}>
                  ...
                  <p className="text-xs lg:text-sm" style={{ color: "#17191c" }}>No bookings yet</p>
                  <p className="text-xs lg:text-smmax-w-xs" style={{ color: "#a3a6af" }}>
