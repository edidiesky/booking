...
export default function RankedListCard({ title, icon, items, emptyLabel = "Nothing to show yet." }: Props) {
  return (
    <div className="border rounded-xl p-4" style={{ borderColor: "#e8e6e3" }}>
      ...
      {items.length === 0 ? (
        <p className="text-xs py-8 text-center" style={{ color: "#a3a6af" }}>{emptyLabel}</p>
      ) : (
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs bold truncate" style={{ color: "#17191c" }}>{item.title}</p>
              ...
              {item.value && <span className="text-xs bold shrink-0" style={{ color: "#17191c" }}>{item.value}</span>}
            </button>
          ))}
        </div>
      )}
