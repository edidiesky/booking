...
interface LineSeries { datakey: string; color: string; label: string; }
...
function CustomTooltip({ active, payload, label, series, isCurrency }: TooltipProps) {
  ...
  return (
    <div style={{ background: "white", border: "0.5px solid #e8e6e3", padding: "10px 14px", minWidth: "220px", display: "flex", flexDirection: "column", gap: "8px", boxShadow: "0 2px 8px 0 rgba(0,0,0,0.08)" }}>
      <p style={{ fontSize: "15px", color: "#17191c", margin: 0 }}>{formatDate(label ?? "")}</p>
      {payload.map((entry) => {
        ...
        return (
          <p key={entry.dataKey} style={{ fontSize: "15px", margin: 0 }}>
            ...
            <span style={{ color: "#17191c" }}>{isCurrency ? formatCurrency(entry.value) : entry.value.toLocaleString("en-NG")}</span>
          </p>
        );
      })}
    </div>
    ...
    <div className="flex items-center justify-center gap-x-4 gap-y-2 flex-wrap pt-3 pb-1">
      {series.map((s) => (
        <div key={s.datakey} className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-[#4c4c4c]">{s.label}</span>
        </div>
      ))}
    </div>
  );
...
function ChartBody({ data, chartConfig, series, isCurrency, emptyMessage }: {
  data: LineChartMultipleProps["data"];
  chartConfig: ChartConfig;
  series: LineSeries[];
  isCurrency: boolean;
  emptyMessage: string;
    ...
    <div className="px-2 py-4">
      <ChartContainer config={chartConfig} className="w-full h-[280px] lg:h-[320px]">
        <LineChart data={data} accessibilityLayer>
          ...
          <YAxis tickLine={false} axisLine={false} tickMargin={8} width={isCurrency ? 70 : 55} tick={{ fontSize: 14, fill: "#777b86" }} tickFormatter={(v: number) => isCurrency ? formatCurrency(v) : v.toLocaleString("en-NG")} />
          <ChartTooltip cursor={{ stroke: "#e8e6e3", strokeDasharray: "4" }} content={<CustomTooltip series={series} isCurrency={isCurrency} />} />
          {series.map((s) => (
            <Line key={s.datakey} dataKey={s.datakey} type="monotone" stroke={s.color} strokeWidth={2} dot={false} activeDot={{ r: 4, fill: s.color, strokeWidth: 0 }} />
          ))}
        </LineChart>
      </ChartContainer>
