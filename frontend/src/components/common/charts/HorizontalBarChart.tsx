...
function CustomTooltip({ active, payload, series, isCurrency }: TooltipProps) {
  ...
  return (
    <div className="border border-[#e8e6e3] bg-white rounded-lg p-3 flex flex-col gap-1.5 min-w-[180px]">
      <p className="text-sm text-[#17191c] truncate max-w-[180px]">{label}</p>
      <div className="flex flex-col gap-1">
        <div className="flex items-center justify-between gap-4">
          <span className="text-xs text-[#777b86]">Total</span>
          <span className="text-sm text-[#17191c]">{isCurrency ? `₦${total.toLocaleString("en-NG")}` : total.toLocaleString("en-NG")}</span>
        </div>
        {payload.map((entry) => {
          ...
          return (
            <div key={entry.dataKey} className="flex items-center justify-between gap-4">
              ...
              <span className="text-base text-[#17191c]">{isCurrency ? `₦${entry.value.toLocaleString("en-NG")}` : entry.value.toLocaleString("en-NG")}</span>
            </div>
          );
        })}
      </div>
