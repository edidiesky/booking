...
export default function LinearTickBarCard({ title, totalValue, trend, distributionLabel = "Distribution", segments, icon }: Props) {
  ...
  return (
    <div className="border rounded-xl p-4" style={{ borderColor: "#e8e6e3" }}>
      ...
      <p className="text-xs lg:text-sm mt-4 mb-2" style={{ color: "#17191c" }}>{distributionLabel}</p>
      <div className="relative">
        {hoveredIdx !== null && (
          <div
            className="absolute -top-9 bg-white border rounded-lg shadow-lg px-2.5 py-1.5 z-10 whitespace-nowrap text-xs lg:text-sm pointer-events-none"
            style={{
              borderColor: "#e8e6e3",
              color: "#17191c",
