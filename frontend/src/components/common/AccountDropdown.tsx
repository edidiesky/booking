    ...
    <DropdownMenu>
      ...
      >
        ...
        <DropdownMenuItem asChild>
          ...
          >
            <span
              className="w-9 h-9 rounded-full flex items-center justify-center text-xs bold shrink-0"
              style={{ backgroundColor: "var(--color-ink)", color: "var(--color-canvas)" }}
            >
              {initial}
            </span>
            <div className="flex flex-col leading-tight min-w-0">
              <span className="text-xs bold text-[#17191c] truncate">{fullName || "My Account"}</span>
            ...
            {groups[groupKey].map((item) => {
              ...
              return (
                <DropdownMenuItem key={item.to} asChild>
                  <Link
                    to={item.to}
                    className="flex items-center gap-3 px-2.5 py-2 text-xs lg:text-xs font-semibold text-[#17191c] cursor-pointer hover:bg-[#f2f0ed] rounded-lg outline-none"
                  >
        ...
        <DropdownMenuItem
          ...
          className="flex items-center gap-3 px-2.5 py-2 text-xs lg:text-xs bold text-red-600 cursor-pointer hover:bg-red-50 rounded-lg outline-none"
        >
