...
import { redisClient, logger, requestContext } from "@booking/shared";
...
export abstract class BaseNotificationHandler {
  ...
  async process(data: unknown, channel: Channel, msg: ConsumeMessage): Promise<void> {
    const key = this.idempotencyKey(data);
    const acquired = await redisClient.set(key, "1", "EX", IDEMPOTENCY_TTL_SEC, "NX");

    if (!acquired) {
      logger.info("notification_duplicate_skipped", { event: "notification_duplicate_skipped", routingKey: this.routingKey, key });
      channel.ack(msg);
      return;
    }
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        logger.info("notification_handler_success", {
          event: "notification_handler_success", routingKey: this.routingKey, attempt: attempt + 1, ...requestContext.get(),
        });
        channel.ack(msg);
        return;
      } catch (err) {
        ...
        logger.error("notification_handler_attempt_failed", {
          event: "notification_handler_attempt_failed", routingKey: this.routingKey, attempt: attempt + 1, error: message,
        });

        if (attempt === MAX_RETRIES - 1) {
          logger.error("notification_handler_exhausted", { event: "notification_handler_exhausted", routingKey: this.routingKey });
