import { createScheduleIndex, createLockedScheduler, bookingRepository, logger } from "@booking/shared";
...
async function tick(): Promise<void> {
  ...
  for (const bookingId of due) {
    try {
      ...
      logger.error("booking_expiry_processing_failed", { event: "booking_expiry_processing_failed", bookingId, error: (err as Error).message });
      // re-add for one more attempt shortly, bounded retry via a short re-schedule
      void scheduleIndex.add(bookingId, Date.now() + 60_000);
    }
  }

  logger.info("booking_expiry_tick_complete", { event: "booking_expiry_tick_complete", claimedCount: due.length });
}

const scheduler = createLockedScheduler({
  lockKey: "scheduler:booking_expiry:lock", lockTtlSec: 15, tickMs: 30_000,
  serviceName: "booking-expiry-worker", onTick: tick,
});
