import { createLockedScheduler, query, availabilityBroadcaster, logger, locksPurgedCounter, driftDetectedGauge } from "@booking/shared";

async function lockSweepTick(): Promise<void> {
  const expired = await query<{ room_type_id: string; check_in: string; check_out: string }>(
...
async function reconciliationTick(): Promise<void> {
  const drifted = await query<{ room_type_id: string; date: string; computed_count: number }>(
    `SELECT ac.room_type_id, ac.date, rt.quantity - COALESCE(active.held, 0) AS computed_count
     ...
     LEFT JOIN LATERAL (
       ...
       WHERE bl.room_type_id = ac.room_type_id AND ac.date >= bl.check_in AND ac.date < bl.check_out
     ) active ON true
...
export const lockSweepScheduler = createLockedScheduler({
  lockKey: "scheduler:lock_sweep:lock", lockTtlSec: 15, tickMs: 5 * 60_000,
  serviceName: "availability-worker", onTick: lockSweepTick,
});
